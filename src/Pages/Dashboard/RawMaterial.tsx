...
const StandardRecipe = () => {
  const [recipes, setRecipes] = useState<RawMaterial[]>(initialRecipes);
  const [isViewModalVisible, setIsViewModalVisible] = useState<boolean>(false);
  const [isModalVisible, setIsModalVisible] = useState<boolean>(false);
  const [isEditMode, setIsEditMode] = useState<boolean>(false);
  const [selectedRecipe, setSelectedRecipe] = useState<RawMaterial | null>(null);
  const [form] = Form.useForm();
  const [rowMaterials, setRowMaterials] = useState<Material[]>([]);
  const [newMaterial, setNewMaterial] = useState<string>("");
  const [newQuantity, setNewQuantity] = useState<string>("");

  const showModal = (record: RawMaterial) => {
  ...
  const showEditModal = (record: RawMaterial) => {
    setIsEditMode(true);
    setSelectedRecipe(record);
